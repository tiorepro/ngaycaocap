...
window.handleViewResult = async function handleViewResult() {
    ...
    try {
        ...
        document.getElementById('results-container').innerHTML = '<div class="m-empty-state"><span style="font-size:48px;">🔍</span><p>Hãy qua màn hình Bộ Lọc và nhấn "ÁP DỤNG".</p></div>';
        ...
        window.showToast('✅ Đã có thông tin!');
        overlay.style.display = 'none';
...
window.renderInputCards = function renderInputCards() {
    ...
    document.getElementById('card-tuoi').innerHTML = `<div class="m-info-row"><span class="m-info-label">Can Chi</span><span class="m-info-value">${canChi}</span></div><div class="m-info-row"><span class="m-info-label">Lục Thập Hoa Giáp</span><span class="m-info-value">${LUC_THAP_NAP_AM_MAP[canChi] || 'N/A'}</span></div><div class="m-info-row"><span class="m-info-label">Lạc Thư Hoa Giáp</span><span class="m-info-value">${LAC_THU_NAP_AM_MAP[canChi] || 'N/A'}</span></div><div class="m-info-row m-info-row--col"><span class="m-info-label">Quẻ HKĐQ</span>${renderGuaVisual(canChi, d.hanhTuoiArr, d.vanTuoiArr)}</div>`;
    ...
    document.getElementById('card-nam').innerHTML = `<div class="m-info-row"><span class="m-info-label">Năm</span><span class="m-info-value text-gold">${d.viewYear} – ${ycc}</span></div><div class="m-info-row"><span class="m-info-label">Lục Thập Hoa Giáp</span><span class="m-info-value">${LUC_THAP_NAP_AM_MAP[ycc] || 'N/A'}</span></div><div class="m-info-row"><span class="m-info-label">Lạc Thư Hoa Giáp</span><span class="m-info-value">${LAC_THU_NAP_AM_MAP[ycc] || 'N/A'}</span></div><div class="m-info-row m-info-row--col"><span class="m-info-label">Quẻ HKĐQ</span>${renderGuaVisual(ycc, d.hanhNamArr, d.vanNamArr)}</div><div class="m-info-row"><span class="m-info-label">Ngũ Hoàng</span><span class="m-info-value text-warning">${s.nguHoangNam} – ${(palaceToSonMap[s.nguHoangNam]||[]).join(', ')}</span></div><div class="m-info-row"><span class="m-info-label">Nhị Hắc</span><span class="m-info-value">${s.nhiHacNam} – ${(palaceToSonMap[s.nhiHacNam]||[]).join(', ')}</span></div><div class="m-info-row"><span class="m-info-label">Thái Tuế</span><span class="m-info-value">${s.thaiTue}</span></div><div class="m-info-row"><span class="m-info-label">Xung Thái Tuế</span><span class="m-info-value text-danger">${s.tuePha}</span></div><div class="m-info-row"><span class="m-info-label">Tam Sát</span><span class="m-info-value">${getDetailedTamSatInfo(s.yearChi)}</span></div><div class="m-info-row"><span class="m-info-label">Bát Sát</span><span class="m-info-value text-danger">${BAT_SAT_NAM_CHI_MAP[s.yearChi] || 'Không có'}</span></div>`;
...
function createFilterGiaDinh() {
    ...
    if (!container) return;
    const families = [ { name: 'Càn - Khôn', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Khảm - Ly', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Chấn - Tốn', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Cấn - Đoài', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Bĩ - Thái', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Ký Tế - Vị Tế', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Hằng - Ích', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] }, { name: 'Tổn - Hàm', roles: ['Cha', 'Mẹ', 'Nam tử', 'Nữ tử'] } ];
...
window.renderDayCard = function renderDayCard(dayInfo, idx) {
    ...
    dayInfo.passingHours.forEach((h, index) => {
        ...
        const qnLabel = h.thienAt ? (h.thienAt.indexOf('Dương') === 0 ? 'DQN' : 'AQN') : '';
        const qnHtml = qnLabel ? `<span class="m-hour-qn" title="Thiên Ất Quý Nhân (${h.thienAt})">★${qnLabel}</span>` : '';
        const titleHtml = `<div class="m-hour-title" onclick="window.toggleHourSelect(event, '${hourKey}', '${h.chi}', this)">
                ...
                <span class="m-hour-score">🔆 ${h.pillarScore6}</span>
        ...
        <div class="m-day-card ${isSelected ? 'selected' : ''}" id="day-card-${jdn}">
            <div class="m-day-header" onclick="window.toggleDayExpand(${jdn})">
                ...
                <div class="m-day-header-bot">
                    ...
                    <span class="m-day-score"><strong>🔆 </strong> ${dayInfo.pillarScore5 ?? 0}</span>
                </div>
            </div>
...
// ==================== PRINT ====================
// [MỚI] Bảng chi tiết cho 1 ngày trong bản IN — giống hệt cấu trúc bảng ở tab Kết Quả (Can Chi +
// Quẻ HKĐQ của Tuổi/Tọa/Ngày/Tháng/Năm + các Giờ), NHƯNG: (1) chỉ đưa vào những Giờ đã được chọn
// (bỏ Giờ chưa chọn), (2) không có dòng Gia Đình/Vai Trò/Ghi chú theo yêu cầu.
function buildPrintDayTable(dayInfo) {
    const jdn = dayInfo.jdn;
    const d = MOBILE_STATE.inputData;
    const truNen = dayInfo._truNen;
    const cols = [];
    cols.push({ title: 'TUỔI', ...renderSinglePillarCell(d.birthInfo.canChi, truNen && truNen.ketQua['Trụ Tuổi']) });
    cols.push({ title: 'TỌA', ...renderSinglePillarCell(d.toaInfo.canChi, truNen && truNen.ketQua['Trụ Tọa']) });
    cols.push({ title: 'NGÀY', ...renderSinglePillarCell(dayInfo.dayCanChi, truNen && truNen.ketQua['Trụ Ngày']) });
    cols.push({ title: 'THÁNG', ...renderSinglePillarCell(dayInfo.thangCanChiTK, truNen && truNen.ketQua['Trụ Tháng']) });
    cols.push({ title: 'NĂM', ...renderSinglePillarCell(dayInfo.namCanChiTK, truNen && truNen.ketQua['Trụ Năm']) });

    (dayInfo.passingHours || []).forEach(h => {
        if (!MOBILE_STATE.selectedHours[`${jdn}_${h.chi}`]) return; // chỉ Giờ đã chọn
        const qnLabel = h.thienAt ? (h.thienAt.indexOf('Dương') === 0 ? 'DQN' : 'AQN') : '';
        const qnHtml = qnLabel ? `<span class="m-hour-qn">★${qnLabel}</span>` : '';
        const titleHtml = `<div class="m-hour-title">
                <span class="m-hour-name">GIỜ ${h.chi.toUpperCase()} ${qnHtml}</span>
                <span class="m-hour-range">${h.hourRange || ''}</span>
                <span class="m-hour-score">🔆 ${h.pillarScore6}</span>
            </div>`;
        cols.push({ title: titleHtml, ...renderSinglePillarCell(h.hourCanChi, h.ketQuaGio) });
    });
    ...
    return `
        <div class="m-print-day">
            <div class="m-print-day-head">
                <span class="m-print-day-date">${solarDate} (${dayOfWeek}) · Âm Lịch: ${lunarDate}</span>
                <span class="m-print-day-meta">${dayInfo.dayCanChi} · Tiết: ${dayInfo.tietKhi} · Chính Ngọ: ${dayInfo.chinhNgo || 'N/A'}</span>
            </div>
            <div class="m-hkdq-table-wrap">
                <table class="m-hkdq-table">
                    <thead>
...
window.printSelected = function printSelected() {
    ...
    // sách ngày giờ đã chọn [khối này — bảng chi tiết đầy đủ như tab Kết Quả, chỉ Giờ đã chọn,
    // không có dòng Gia Đình/Vai Trò/Ghi chú], (3) ấn triện + chữ ký [tĩnh, có sẵn trong index.html].
    ...
    selectedDays.sort((a, b) => a - b).forEach(jdn => {
        const dayInfo = MOBILE_STATE.filteredDates.find(d => d.jdn === jdn) || MOBILE_STATE.allDates.find(d => d.jdn === jdn);
        if (!dayInfo || !dayInfo.passingHours) return;
        const hasSelectedHour = dayInfo.passingHours.some(h => MOBILE_STATE.selectedHours[jdn + '_' + h.chi]);
        if (!hasSelectedHour) return;
        html += buildPrintDayTable(dayInfo);
    });
    printContent.innerHTML = html;

    // [MỚI] Trước đây ấn triện cố định 96px trong khi chữ "TIEN.TQN" được ép giãn bằng CSS
    // (inter-character justify) — kỹ thuật này không phải trình duyệt/engine in nào cũng hỗ trợ
    // tốt với chữ Latin, khiến ấn triện nhìn to hơn chữ. Nay đo TRỰC TIẾP độ rộng thật của chữ
    // "TIEN.TQN" đã render, rồi đặt ấn triện đúng bằng độ rộng đó — luôn khớp chính xác mọi font.
    const sigBlock = document.querySelector('.m-print-signature');
    const nameEl = document.getElementById('print-signer-name');
    const sealEl = document.querySelector('.m-print-seal');
    if (sigBlock && nameEl && sealEl) {
        const prev = { display: sigBlock.style.display, visibility: sigBlock.style.visibility, position: sigBlock.style.position, left: sigBlock.style.left };
        // Tạm hiện khối chữ ký (đang display:none ngoài lúc in) ở ngoài màn hình để đo đúng kích thước thật
        sigBlock.style.display = 'flex';
        sigBlock.style.visibility = 'hidden';
        sigBlock.style.position = 'fixed';
        sigBlock.style.left = '-9999px';
        const w = nameEl.getBoundingClientRect().width;
        sigBlock.style.display = prev.display;
        sigBlock.style.visibility = prev.visibility;
        sigBlock.style.position = prev.position;
        sigBlock.style.left = prev.left;
        if (w > 0) { sealEl.style.width = w + 'px'; sealEl.style.height = w + 'px'; }
    }
