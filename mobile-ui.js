        ...
        try {
            ...
            for (const dateInfo of MOBILE_STATE.allDates) {
                ...
                for (const h of hours) {
                    ...
                    passingHours.push({ chi: h.chi, hour: h.hour, hourCanChi: hourCanChiText, hanhGioArr, vanGioArr, totalScore, bestHanhRel, bestVanRel });
                }
...
function renderSinglePillarCell(canChi) {
    ...
    if (ques.length === 0) {
        ...
        ques.forEach(qName => {
            const ttList = (typeof timThongTinQue === 'function') ? timThongTinQue(qName) : [];
            const tt = ttList.length > 0 ? ttList[0] : null;
            if (tt) {
...
window.renderDayCard = function renderDayCard(dayInfo, idx) {
    ...
    const cols = [];
    cols.push({ title: 'TUỔI', ...renderSinglePillarCell(d.birthInfo.canChi) });
    cols.push({ title: 'TỌA', ...renderSinglePillarCell(d.toaInfo.canChi) });
    cols.push({ title: 'NGÀY', ...renderSinglePillarCell(dayInfo.dayCanChi) });
    cols.push({ title: 'THÁNG', ...renderSinglePillarCell(dayInfo.thangCanChiTK) });
    cols.push({ title: 'NĂM', ...renderSinglePillarCell(dayInfo.namCanChiTK) });
    
    dayInfo.passingHours.forEach((h, index) => {
        const hourKey = `${jdn}_${h.chi}`;
        const isHourSelected = MOBILE_STATE.selectedHours[hourKey];
        const titleHtml = `<div style="cursor:pointer;" onclick="window.toggleHourSelect(event, '${hourKey}', '${h.chi}', this)">
                GIỜ ${h.chi.toUpperCase()} <br> 
                <span style="font-size: 0.85rem; font-weight:normal;" id="chk-hr-${hourKey}">${isHourSelected ? '✅' : '☐'}</span>
            </div>`;
        cols.push({ title: titleHtml, ...renderSinglePillarCell(h.hourCanChi) });
    });
...
window.printSelected = function printSelected() {
    ...
    selectedDays.forEach(jdn => {
        ...
        html += `<div style="margin-bottom:12px;padding:8px;border:1px solid #ddd;"><strong>${solarDate}</strong> – ${dayInfo.dayCanChi} – ÂL: ${dayInfo.lunarDay}/${dayInfo.lunarMonth}/${dayInfo.lunarYear}<br>Tiết khí: ${dayInfo.tietKhi || '-'}<br><em>Giờ tốt: ${hoursForDay.join(', ')}</em></div>`;
    });
