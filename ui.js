...
// ---------- CHỌN QUẺ PHÂN TÍCH HKĐQ ----------
// Mỗi Can Chi có thể có 2 quẻ.
// Gán vai trò: 1 quẻ/trụ — ưu tiên Phụ Mẫu; không có PM thì chọn Tử Tức
// đúng 1 họ có bằng chứng trên trụ khác.
...
function chonQuePhanTich(canChi, hoBangChung) {
    ...
    const unique = ques.filter(q => {
        const matched = giaDinhCuaQue(q).filter(gd => hoBangChung && hoBangChung.has(gd));
...
function renderHkdqAnalysis(ketQua) {
    ...
    ['Trụ Tuổi','Trụ Tọa','Trụ Ngày','Trụ Tháng','Trụ Năm','Trụ Giờ'].forEach(tenTru => {
        ...
        if (kq && kq.tenQue) {
            ...
            if (kq.queConLai && kq.queConLai.length) {
                html += `<small style="color:#999;font-size:8px;">(còn: ${kq.queConLai.join(', ')})</small>`;
            }

            if (tt && kq.trangThai !== 'KXĐ' && kq.trangThai !== 'Không tìm thấy') {
                html += `<span class="am-duong ${tt.amDuong==='Dương'?'duong':'am'}" style="padding:2px 6px;font-size:0.85em;border-radius:10px;">${tt.amDuong}</span><br>`;
                const roleClass = kq.trangThai === 'Phụ Mẫu' ? 'pm' : 'tt';
                html += `<span class="hkdq-role ${roleClass}">${kq.trangThai}</span><br>`;
                ...
                html += '<div class="kxd-info">';
                html += `<div class="kxd-title">${kq.lyDoKXDLabel || 'Không xác định'}</div>`;
                if (kq.tatCaThongTin && kq.tatCaThongTin.length > 0) {
                    kq.tatCaThongTin.forEach(info => {
                        const isGoiY = kq.goiYThanhThe && info.giaDinh === kq.goiYThanhThe;
                        html += `<div class="kxd-item"><span class="kxd-family">${info.giaDinh}${isGoiY ? ' ★' : ''}</span><span class="kxd-badge ${info.amDuong==='Dương'?'duong':'am'}">${info.amDuong}</span></div>`;
                    });
                }
                if (kq.lyDoKXD === 'dung_nhieu_ho' && kq.goiYThanhThe) {
                    html += `<div style="margin-top:2px;color:#ef6c00;font-size:8px;">Tham khảo (không gán): ${kq.goiYThanhThe}</div>`;
                }
                html += '<div class="kxd-warning">⚠️ Khí tạp, không dùng Âm Dương trụ này</div></div>';
            }
        } else {
    ...
    if (hd.tongHuynhDe > 0 && Array.isArray(hd.danhSachHuynhDe)) {
        hd.danhSachHuynhDe.forEach(nhom => {
            const ds = (nhom.cacTru || []).map((tru, i) =>
                `${String(tru).replace('Trụ ','')}-${nhom.cacQue[i]}`
            ).join(', ');
            html += `${nhom.giaDinh}: ${nhom.soLuong} quẻ (${ds})<br>`;
        });
    } else {
        html += '<em>Không có Tử Tức cùng họ</em>';
    }
    html += '</div>';
...
function populateAnalysisTable(hourCell) {
    ...
    const canChiTheoTru = {
        'Trụ Tuổi': row.dataset.canchiTuoi,
        'Trụ Tọa': row.dataset.canchiToa,
        'Trụ Ngày': row.dataset.canchiNgay,
        'Trụ Tháng': row.dataset.canchiThang,
        'Trụ Năm': row.dataset.canchiNamTk,
        'Trụ Giờ': hourCell.dataset.canchiGio
    };

    const tatCaQueTheoTru = {};
    Object.entries(canChiTheoTru).forEach(([tenTru, canChi]) => {
        tatCaQueTheoTru[tenTru] = layDanhSachQue(canChi);
    });

    const chonTheoTru = {};
    Object.entries(canChiTheoTru).forEach(([tenTru, canChi]) => {
        const hoBangChung = thuThapHoBangChung(tenTru, tatCaQueTheoTru);
        chonTheoTru[tenTru] = chonQuePhanTich(canChi, hoBangChung);
    });

    const lucXungList = [];
    const xungPairs = [
        ['Tuổi-Tọa', chi.tuoi, chi.toa],
        ['Tuổi-Ngày', chi.tuoi, chi.ngay],
        ['Tọa-Ngày', chi.toa, chi.ngay],
        ['Ngày-Giờ', chi.ngay, chi.gio],
        ['Ngày-Tháng', chi.ngay, chi.thang],
        ['Ngày-Năm', chi.ngay, chi.nam],
        ['Tháng-Năm', chi.thang, chi.nam]
    ];
    xungPairs.forEach(([ten, c1, c2]) => {
    ...
    Object.entries(chonTheoTru).forEach(([tenTru, chon]) => {
        if (ketQua.ketQuaCacTru[tenTru]) {
            ketQua.ketQuaCacTru[tenTru].queConLai = chon.queConLai;
        }
    });

    document.getElementById('hkdq-analysis-cell').innerHTML = renderHkdqAnalysis(ketQua);
