...
// Bằng chứng huyết thống = mọi thành viên cùng họ trên 5 trụ còn lại
// (Phụ Mẫu / Tử Tức / Huynh Đệ đều là bằng chứng).
// Bỏ qua chính trụ và bỏ qua quẻ trùng tên — quẻ lưỡng họ không được
// tự làm bằng chứng cho chính nó.
function quanXetQueTheoNguCanh(tenQueCanXet, tenTruCanXet, tatCaCacTru) {
    ...
    if (laTuTuc) {
        ...
        for (const [tenTru, tenQue] of Object.entries(tatCaCacTru)) {
            if (tenTru === tenTruCanXet || !tenQue || tenQue === tenQueCanXet) continue;
    ...
    // BƯỚC 3: fallback (database hiện tại mọi quẻ đều PM hoặc TT)
    return {
        thongTinDuocChon: thongTinList[0],
        trangThai: 'Tử Tức',
        tatCaThongTin,
        bangChungGiaDinh: thongTinList[0] ? [thongTinList[0].giaDinh] : [],
        lyDoKXD: null
    };
}
...
function phanTichNhatKhoaDayDu(input) {
    ...
    for (const [tenTru, tenQue] of Object.entries(cacTruInput)) {
        ...
        const ketQua = quanXetQueTheoNguCanh(tenQue, tenTru, cacTruInput);
