...
// ---------- CHÍNH NGỌ (ĐỒNG BỘ - dùng longitude đã cache sẵn) ----------
// [MỚI] Dùng để tính nhanh Giờ Chính Ngọ cho từng ngày trong danh sách kết quả
// mà không cần gọi lại API định vị (longitude đã được getSolarNoon() nạp/cache trước đó).
function getSolarNoonSync(jdn) {
    if (longitudeCache === null) return "N/A";
    if (longitudeCache === 'error') return "N/A (Lỗi)";
    const eotMinutes = calculateEquationOfTime(jdn);
    const solarNoonDecimalHours = 12 - (longitudeCache / 15) - (eotMinutes / 60) + TIME_ZONE;
    const hours = Math.floor(solarNoonDecimalHours);
