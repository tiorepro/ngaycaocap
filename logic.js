...
// ✅ SỬA LỖI 2: Đảm bảo gom chính xác dựa trên thuộc tính trangThai (Bao gồm cả 'Huynh Đệ' mồ côi)
...
// --- THUẬT TOÁN LOGIC QUÉT VÀ ĐÁNH GIÁ 2 BƯỚC (CÓ TRỌNG SỐ THANH THẾ) ---



// BƯỚC 1: Hàm đánh giá tối ưu cho từng quẻ

// ✅ SỬA LỖI 1 & 3: Thêm tham số Tie-breaker (tanSuatGiaDinhTuTuc) và kiểm tra phòng thủ Âm Dương
...
function xetVaiTroToiUu(tenQue, diemThanhThe, tanSuatGiaDinhTuTuc) {
    ...
    // Defensive Programming: Đảm bảo tính nhất quán của Âm Dương trong dữ liệu đầu vào

    const firstAmDuong = thongTinList[0].amDuong;

    const hasConflict = thongTinList.some(tt => tt.amDuong !== firstAmDuong);

    if (hasConflict) {

        console.warn(`CẢNH BÁO DỮ LIỆU: Quẻ ${tenQue} có sự bất nhất về giới tính Âm/Dương giữa các gia đình!`);

    }



    // Ưu tiên 1: Phụ Mẫu luôn là Phụ Mẫu
    ...
    // Tách riêng các lựa chọn Tử Tức
    ...
    if (cacLuaChonTuTuc.length > 0) {
        ...
        // SẮP XẾP CÁC LỰA CHỌN THEO ĐIỂM THANH THẾ (TỪ CAO XUỐNG THẤP)
        ...
        cacLuaChonTuTuc.sort((a, b) => {
            ...
            // TIE-BREAK: Điểm bằng nhau thì chọn phe có số Tử Tức đông hơn
        ...
        // Nếu gia đình này có điểm >= 10, nghĩa là CHẮC CHẮN có Phụ Mẫu chống lưng

        if (diemCuaLuaChon >= 10) {

            return { thongTinDuocChon: luaChonTotNhat, trangThai: 'Tử Tức' };

        }

        

        // Nếu điểm từ 2 đến 9, nghĩa là không có Phụ Mẫu, nhưng có bầy đàn Huynh Đệ

        if (diemCuaLuaChon >= 2) {

            return { thongTinDuocChon: luaChonTotNhat, trangThai: 'Huynh Đệ' };
...
// BƯỚC 2: Bộ máy thẩm định tính điểm hấp dẫn 6 Trụ
...
function phanTichNhatKhoaDayDu(input) {
    ...
    // BẢNG ĐIỂM THANH THẾ CỦA CÁC GIA ĐÌNH VÀ TẦN SUẤT
    ...
    // LẦN QUÉT 1: Tính toán "Thế Lực" và đếm "Tần suất Tử Tức" của từng gia đình
    ...
    for (const [tenTru, tenQue] of Object.entries(cacTruInput)) {
        ...
        // Trọng số vị trí: Nằm ở Tam Tài (Tuổi, Tọa, Ngày) thì mạnh gấp đôi
        ...
        // Đếm phục vụ thuật toán Tie-Break
    ...
    // LẦN QUÉT 2: Gán vai trò dựa trên Bảng điểm thanh thế & Tie-break
...
// --- BẢNG ĐIỂM TRỌNG SỐ CHO HÀNH VÀ VẬN ---

const SCORE_MAP = { "Hợp Thập": 10, "Hợp Ngũ": 8, "Hợp Thập Ngũ": 8, "Cùng Quái": 8, "Hà Đồ": 6, "Điên Đảo Ai Tinh": 6, "Sinh Nhập": 5, "Khắc Nhập": 3 };
